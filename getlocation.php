...
$todate =$_POST["todate"];
//creating a query
$stmt = $conn->prepare("SELECT r.roomid, r.hotelid, r.roomtype, r.price, r.size, h.hotelname, h.address, h.location, h.phoneno, h.email, h.rating, b.from_date, b.to_date FROM room r, hotel h, bookedroom b WHERE h.id = r.hotelid AND b.roomid = r.roomid AND r.roomid NOT IN(SELECT roomid from bookedroom WHERE from_date <= '$todate' AND to_date >= '$fromdate') AND h.hotelname LIKE '%$name%' AND r.adult >= '$adult' AND r.children >= '$children' GROUP BY r.roomid ORDER BY CAST(r.price AS UNSIGNED) ASC");
